...
<head>
	...
	<script type="text/javascript">
		...
		$(function(){
		    ...
		    $("#addBtn").click(function () { // 直接CV大法，可不用清晰记忆
                $(".time").datetimepicker({
                    minView: "month",
                    language:  'zh-CN',
                    format: 'yyyy-mm-dd',
				...
				$.ajax({
                    ...
                    success : function (data) {
						...
						// 取得当前登录用户的id，在js中使用EL表达式，EL表达式必须要套用在字符串中
                ...
                $.ajax({
                    ...
                    success : function (data) {
                        if(data.success){
							...
							/*
								注意：
									这里拿到了form表单的jquery对象
									对于表单的jquery对象，提供了submit()方法让我们提交表单
									但是表单的jquery对象，没有为我们提供reset()方法让我们重置表单(坑：idea为我们提示了reset()方法)
									虽然jquery对象没有为我们提供reset()方法，但是元素js为我们提供了reset()方法
									所以我们要将jquery对象z转换为原生dom对象
									jquery对象转换为dom对象：
										jquery对象[下标]
									dom对象转换为jquery对象：
										$(dom)
			...
			$("#searchBtn").click(function () {
				/*
					点击查询按钮的时候，我们应该将搜索框中的信息保存起来（hidden隐藏域）
				 */
		...
		/*
			对于所有的关系型数据库，做前端的分页相关操作的基础组件，就是pageNo和pageSize
			pageNo：页码
			pageSize：每页展现的记录数
			pageList方法：就是发出ajax请求到后台，从后台取得最新的市场活动信息列表数据，通过响应回来的数据，局部刷新市场活动信息列表
			我们在哪些情况下，需要调用pageList方法（什么情况下需要刷新一下市场活动列表）
				...
				success : function (data) {
					/*
						data
							...
							一会分页插件需要的：查询出来的总记录数
							{"total":100} int total
							整合：{"total":100,"dataList":[{市场活动1},{2},{3}]}
					 */
					var html = "";
					// 每一个n就是每一个市场活动对象
				...
				<div class="btn-group" style="position: relative; top: 18%;">
					<!--
						...
						现在我们是以属性和属性值的方式写在了button元素中，用来打开模态窗口，但是这样做是有问题的：
							问题在于没有办法对按钮的功能进行扩充
						所以未来的实际项目开发，对于触发模态窗口的操作，一定不要写死在元素当中，应该由我们自己来写js代码来操作
					-->
