...
<head>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			// 单条的复选框
			$("#activitySearchBody").on("click",$("input[name=checkOne]"),function (){
				$("#checkAll").prop("checked",$("input[name=checkOne]").length==$("input[name=checkOne]:checked").length);
			})
			// 为关联市场活动模态窗口中的 搜索框 绑定事件，通过触发回车键（键码13），查询并展现所需市场活动列表
						...
						success :function (data) {
							...
							*/
							...
							$.each(data,function (i,n){
								html += '<tr>';
								html += '<td><input type="checkbox" name="checkOne" value="'+n.id+'"/></td>';
								html += '<td>'+n.name+'</td>';
            ...
            $("#bundBtn").click(function (){
                var $checkOne = $("input[name=checkOne]:checked");
                if ($checkOne.length==0) {
                    ...
                    for (var i = 0; i < $checkOne.length; i++) {
                        param += "&aid="+$($checkOne[i]).val();
                        if(i<$checkOne.length-1){
                            param += "&";
                        }
                    }
					...
					$.ajax({
						...
						success : function (data) {
							...
							if(data.success) {
								// 刷新关联市场活动的列表
								showActivityList();
								// 清除搜索框中的信息 复选框中的√去掉 清空activitySearchBody中的内容
								$("#activityName").val("");
